"""
Chart update callbacks with click-to-drill functionality
"""
from dash import Input, Output, State
from src.data.loader import load_security_data, get_filtered_data
from src.components.charts import (
    create_severity_pie_chart, create_trend_line_chart,
    create_severity_by_week_chart, create_source_bar_chart,
    create_category_treemap, create_top_repos_chart,
    create_custom_chart, create_attack_timeline_heatmap,
    create_risk_gauge
)
...
def register_chart_callbacks(app):
    ...
    def update_all_charts(source_val, severity_val, status_val, team_val, repo_val, n):
        """Update all 6 main dashboard charts"""
        try:
            ...
            return severity_fig, trend_fig, severity_week_fig, source_fig, category_fig, repos_fig
    ...
    def update_table_on_click(sev_click, trend_click, sev_week_click, 
        ...
        try:
            ...
            def update_all_charts(source_val, severity_val, status_val, team_val, repo_val, n):
                """Update main charts and risk gauge."""
                try:
                    df = load_security_data()
                    filtered = get_filtered_data(df, source_val, severity_val,
                                                 status_val, team_val, repo_val)
                    ...
                    print("Rows in filtered:", len(filtered))
                    print("Status unique:", filtered['Status'].unique() if 'Status' in filtered else "No 'Status'")
                    print("Severity unique:", filtered['Severity'].unique() if 'Severity' in filtered else "No 'Severity'")
                    print("Opened_At min/max:", filtered['Opened_At'].min() if 'Opened_At' in filtered else "No 'Opened_At'",
                                        filtered['Opened_At'].max() if 'Opened_At' in filtered else "No 'Opened_At'")
